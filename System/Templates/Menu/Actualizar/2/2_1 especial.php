...
mysql_select_db($database_COnexionGourmet, $COnexionGourmet);
$query_sopa = "SELECT id_plat, nombre_plat, precio FROM platillo where tipo='Ensalada' ORDER BY CAST(id_plat as UNSIGNED)";
...
$colname_quiere = "-1";
if (isset($_GET['id'])) {
  $colname_quiere = $_GET['id'];
}
mysql_select_db($database_COnexionGourmet, $COnexionGourmet);
$query_quiere = sprintf("SELECT id_plat, nombre_plat FROM platillo WHERE id_plat = %s", GetSQLValueString($colname_quiere, "text"));
$quiere = mysql_query($query_quiere, $COnexionGourmet) or die(mysql_error());
$row_quiere = mysql_fetch_assoc($quiere);
$totalRows_quiere = mysql_num_rows($quiere);
...
mysql_select_db($database_COnexionGourmet, $COnexionGourmet);
$query_SopaActual = sprintf("SELECT platillo.id_plat,detalle_menu.id_menu, case platillo.especial when 1 then detalle_menu.nombre_especial when 0 then platillo.nombre_plat  end as nombre FROM platillo, detalle_menu WHERE detalle_menu.id_plat = platillo.id_plat and platillo.tipo='Ensalada' and id_menu = %s ",GetSQLValueString($_GET['id_menu'], "text"));
...
<title>Editar las ensalada</title>
<!-- InstanceEndEditable -->
...
<div class="container">
  ...
  <div class="content"><!-- end .content --><!-- InstanceBeginEditable name="EditRegion3" -->
    <p>Agregar ensalada al menu del dia: <?php echo $fecha = $_GET['fecha']; ?>
    ...
    <div align="center">
      <table width="663" border="0">
        <tr>
          <td><form id="Nombre" name="nombre" method="get" action="2 nombre.php">
          ...
          <td><form id="form2" name="form2" method="get" action="2 clave.php">
      ...
      <form id="Insertar" name="Insertar" method="POST" action="<?php echo $editFormAction; ?>">
        <h3>
          <input type="hidden" name="id_menu" id="id_menu" value="<?php echo $id_menu = $_GET['id_menu'];?>" />
          <input name="id_plat" type="hidden" id="id_plat" value="<?php echo $row_quiere['id_plat']; ?>" />
          <br />
          Deseas agregar <?php echo $row_quiere['nombre_plat']; ?>
          al menu? 
          <br />
Que es <?php echo $row_quiere['nombre_plat']; ?> ? 
          ...
          <td rowspan="5"><table border="1">
            ...
            <?php do { ?>
              <tr>
                ...
                <td><a href="2_1.php?id=<?php echo $row_sopa['id_plat']; ?>&amp;id_menu=<?php echo $id_menu; ?>&amp;fecha=<?php echo $fecha; ?>"><img src="../../../../Img/aceptar-este-hecho-tan-bien-verde-con-exito-icono-8880-96.png" width="35" height="35" alt="Aceptar" /></a><a href="2_1.php?id=<?php echo $row_sopa['id_plat']; ?>&amp;id_menu=<?php echo $id_menu; ?>"></a></td>
                </tr>
