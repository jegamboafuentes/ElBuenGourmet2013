...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO menu (nombre, descripcion, fecha) VALUES (%s, %s, %s)",
 ...
 $insertGoTo = "Seleccionar Platillo0.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_COnexionGourmet, $COnexionGourmet);
$query_ultimafecha = "SELECT fecha FROM menu ORDER BY fecha DESC";
...
$query_UltimoMenu = "SELECT max( id_menu) FROM menu";
$UltimoMenu = mysql_query($query_UltimoMenu, $COnexionGourmet) or die(mysql_error());
$row_UltimoMenu = mysql_fetch_assoc($UltimoMenu);
$totalRows_UltimoMenu = mysql_num_rows($UltimoMenu);
?>
...
<div class="container">
  ...
  <div class="content"><!-- end .content --><!-- InstanceBeginEditable name="EditRegion3" -->
    <p>&nbsp;</p>
    <form action="<?php echo $editFormAction; ?>" method="post" name="preguntaerror" id="preguntaerror">
      <table align="center">
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">Nombre:</td>
          <td><input type="text" name="nombre" value="<?php echo $row_ultimomenu['id_menu']+1 . " Menu" ; ?>" size="32" /></td>
        </tr>
        <tr valign="baseline">
        ...
        <tr valign="baseline">
          ...
          <td><input name="fecha" type="text" value="<?php echo date("d/m/Y"); ?>" size="32" readonly="readonly" /></td>
        </tr>
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">&nbsp;</td>
          <td><input type="<?php if($row_ultimafecha['fecha']==date("d/m/Y")){echo "hidden";}else {echo "submit";}?>" value="Seleccionar platillo" /></td>
        </tr>
      </table>
      <input type="hidden" name="MM_insert" value="form1" />
      <label for="ultimafecha"></label>
      <input name="ultimafecha" type="hidden" id="ultimafecha" value="<?php echo $row_ultimafecha['fecha']; ?>" />
    </form>
    <form id="form3" name="form3" method="post" action="Menuporfecha.php?fecha=<?php echo date("d/m/Y");?>" style="<?php if($row_ultimafecha['fecha']!=date("d/m/Y")){echo "display:none";}else {echo "display:block";}?>" >
      <div align="center">Ya se creo un Menu Hoy desea editarlo?  
        <input type="<?php if($row_ultimafecha['fecha']!=date("d/m/Y")){echo "hidden";}else {echo "submit";}?>" name="button" id="button" value="OK" />
      </div>
    </form>
    <p>&nbsp;</p>
    <form id="form2" name="form2" method="post" action="">
      <label for="textfield"></label>
      <input type="hidden" name="textfield" id="textfield" value ="<?php if($row_ultimafecha['fecha']==date("d/m/Y")){echo "True";}else {echo "false";}?>"/>
    </form>
    <p>&nbsp;</p>
    <p>&nbsp;</p>
    <p>&nbsp;</p>
